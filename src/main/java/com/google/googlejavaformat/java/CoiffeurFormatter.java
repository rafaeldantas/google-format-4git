...
/**
 * Mostly copied from:
 * https://github.com/google/google-java-format/blob/master/core/src/main/java/com/google/googlejavaformat/java/Main.java
 *
 */
public class CoiffeurFormatter {
    ...
    public static void main(String[] args) {
        ...
        if (!gitParameters.useGit()) {
            ...
            JavaFormatterOptions formatterOptions =
                    new JavaFormatterOptions(JavadocFormatter.ECLIPSE, Style.AOSP, SortImports.ALSO) {
                        @Override
    ...
    private int format(JavaFormatterOptions options, GitFormatterParameters gitParameters) throws UsageException {
        ...
        for (FileToFormat fileToFormat : filesToFormat) {
            results.add(executorService.submit(new FormatFileCallable(fileToFormat, outputLock, options, true, out, err)));
        }
        ...
        for (String fileName : git.listModified()) {
            if (fileName.endsWith(".java")) {
                ...
                if (gitParameters.gitModifiedOnlyFlag()) {
                    ...
                    out.println("done listModifiedRanges " + fileName);
                } else {
                ...
                try {
                    ...
                    if (added) {
                        filesToFormat.add(new FileToFormatPath(originalPath, rangeSet, new ArrayList<>(), new ArrayList<>()));
                    }
                } catch (IOException e) {
