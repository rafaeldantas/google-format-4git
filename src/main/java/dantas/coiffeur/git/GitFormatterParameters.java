...
public class GitFormatterParameters {
    ...
    @Parameter(
        names = {"-g", "--g", "-git", "--git"},
        description = "Use git staging area to detrmine the list of files to be formatted"
    )
    boolean gitFlag = false;

    @Parameter(
        names = {"-m", "--m", "-modified-lines", "--modified-lines"},
        description = "Format only modified lines"
    )
    boolean gitModifiedOnlyFlag = false;

    @Parameter(
        names = {"-ml", "--ml", "-max-line-length", "--max-line-length"},
        description = "Format only modified lines"
    )
